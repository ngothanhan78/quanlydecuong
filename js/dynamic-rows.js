...
const DynRows = {
  ...
  _buildCell(col, rowIdx, tableId) {
    ...
    if (col.key === 'STT' || col.key === 'STT_auto') {
    ...
    if (col.type === 'menu' || col.type === 'select') {
      // Tìm datalist: ưu tiên dl-{key}, fallback dl-{sourceKey}
      const dlId  = `dl-${col.key}`;
      const dlId2 = col.sourceKey ? `dl-${col.sourceKey}` : null;
      const dl    = document.getElementById(dlId) ||
                    (dlId2 ? document.getElementById(dlId2) : null);
      if (dl) {
        return `<td class="col-left">
          <input type="text" placeholder="${ph}" list="${dl.id}" autocomplete="off"
            ${this._oninput(col, tableId)}/>
        </td>`;
      }
      // Fallback: select với options từ window._opts_<key>
