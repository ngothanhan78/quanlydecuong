...
const Calc = {
  ...
  calcTC() {
    ...
    // ✅ FIX: Bridge HTML IDs cũ → varName mới trong schema
    // index.html dùng tcLT/tcTH/tcTong nhưng schema dùng tinchiLT/tinchiTH/tinchi
    const ID_BRIDGE = {
      tinchiLT: 'tcLT', tinchiTH: 'tcTH', tinchi: 'tcTong',
    };
    Object.entries(ID_BRIDGE).forEach(([schemaKey, htmlId]) => {
      if (ctx[schemaKey] === undefined) {
        const el = document.getElementById(htmlId);
        if (el) ctx[schemaKey] = parseFloat(el.value) || 0;
      }
    });
  ...
  autoFillLookup(anchorKey, anchorValue) {
    const anchorVal = (anchorValue || '').trim();
    if (!anchorVal) return;
    const map = window[`_lookupMap_${anchorKey}`] || {};
    const data = map[anchorVal];
    ...
    Object.entries(FIELD_SCHEMA)
      ...
      .forEach(([key, f]) => {
        if (!data) return;
        // targetKey: lấy từ lookupFrom (vd: "hocphanMap!tenAnh" → "tenAnh")
        const fromParts = f.lookup.from?.split('!');
        const targetKey = fromParts?.[1] || f.dataKey || key;
        const val = data[targetKey] ?? data[f.dataKey || key] ?? '';
        const el  = document.getElementById(f.formId || key);
        if (el && val !== undefined) {
          el.value = val;
