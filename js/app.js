...
function startNewForm() {
  ...
  Form.reset();
  _resetHoanThanhBtn();
  // ✅ Điền lại chunhiem sau reset (Form.reset() xóa trắng field)
...
function openHuongDan() {
  const url = ENV_CONFIG.HUONG_DAN_URL || '#';
  if (url === '#') {
...
async function loadMenuData() {
  ...
  fillDatalist('dl-hoTen',         d.giangVien    || []);
  ...
  if (d.hocKyDT && d.hocKyDT.length) fillSelect('hocKy',   d.hocKyDT, true);
  if (d.khoaDT  && d.khoaDT.length)  fillSelect('khoaDT',  d.khoaDT,  true);
  if (d.namHoc  && d.namHoc.length)   fillSelect('namHocApDung', d.namHoc, true);
  ...
  // ✅ FIX: Set lookup maps đúng key để autoFillLookup() tìm được
  // Map tenTV → { tenTA, maHP, maTQ } dùng cho autoFillHocphan
  if (d.hocphanMap) {
    window._lookupMap_tenTV = {};
    Object.entries(d.hocphanMap).forEach(([tenViet, info]) => {
      window._lookupMap_tenTV[tenViet] = {
        tenTA : info.tenAnh  || '',
        maHP  : info.maHP    || '',
        maTQ  : info.maTQ    || '',
      };
    });
    // Cũng lưu _hocphanMap để tương thích
    window._hocphanMap = d.hocphanMap;
  }

  // Map hoTen → { email, donviCT } dùng cho lookup GV trong bảng
  if (d.giangVienEmail) {
    window._lookupMap_hoTen = {};
    (d.giangVien || []).forEach(ten => {
      window._lookupMap_hoTen[ten] = {
        email  : d.giangVienEmail?.[ten] || '',
        donviCT: d.giangVienDonvi?.[ten] || '',
      };
    });
  }

  // ✅ FIX: Thêm datalist riêng cho HP tiên quyết/trước/song hành (TOÀN BỘ HP)
  // Tách khỏi dl-hocphan-viet (filtered) để user thấy tất cả HP
  const allHP = d.hocphanAll || d.hocphanViet || [];
  fillDatalist('dl-hocphan-all', allHP);
}
...
function showApp(user) {
  ...
  // v6: load menu data filtered by user + auto-fill cnHp
  if (typeof App !== 'undefined' && App.boot) App.boot(user);
  ...
  if (gasInput) gasInput.value = ENV_CONFIG.GAS_URL || '';
...
window.addEventListener('DOMContentLoaded', () => {
  window._GAS_URL = ENV_CONFIG.GAS_URL;
