...
//
//  Thay đổi so với v5:
//  • Sau login thành công → gọi showApp(user)
//  • showApp() tự gọi loadMenuData() + App.boot() để filter Hocphan
...
const Auth = {
  ...
  async login(){
    ...
    if(res?.success){
      ...
      this._saveSession(user);
      // ✅ v6: showApp() tự gọi loadMenuData() với userName để filter Hocphan
      showApp(user);
    } else {
