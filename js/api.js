...
const API = {
  ...
  isConfigured() {
    const url = window.ENV_CONFIG.GAS_URL || '';
  ...
  async call(payload, timeoutMs) {
    timeoutMs = timeoutMs || ENV_CONFIG.GAS_TIMEOUT_DEFAULT;
    ...
    return new Promise(resolve => {
      ...
      s.src      = `${ENV_CONFIG.GAS_URL}?data=${enc}&callback=${cb}`;
  ...
  async post(payload) {
    ...
    try {
      const res  = await fetch(ENV_CONFIG.GAS_URL, {
        method : 'POST',
